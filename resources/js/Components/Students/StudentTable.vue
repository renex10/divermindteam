    ...
    <table class="min-w-full divide-y divide-gray-300">
      ...
      <tbody class="bg-white divide-y divide-gray-200">
        ...
        <tr v-for="student in studentsData" :key="student.id" class="hover:bg-gray-50">
            ...
            <div class="flex items-center">
              ...
              <div class="ml-4">
                <div class="text-sm font-medium text-gray-900">
                  {{ student.display_name || student.user?.full_name || `${student.user?.name || ''}
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            <span :class="needTypeBadgeClass(student.need_type)"
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            <span :class="priorityBadgeClass(student.priority)"
              class="inline-flex px-2 py-1 text-xs font-semibold rounded-full">
          ...
          <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
            <div class="flex items-center justify-end space-x-2">
              <!-- Botón Ver -->
              <button @click="$emit('view', student)" class="text-indigo-600 hover:text-indigo-900 p-1 rounded"
                title="Ver detalles">
                <EyeIcon class="w-4 h-4" />
              </button>

              <!-- Botón Editar -->
              <button @click="$emit('edit', student)" class="text-yellow-600 hover:text-yellow-900 p-1 rounded"
                title="Editar">
                <PencilIcon class="w-4 h-4" />
              </button>

              <!-- Botón Eliminar -->
              <button @click="$emit('delete', student)" class="text-red-600 hover:text-red-900 p-1 rounded"
                title="Eliminar">
